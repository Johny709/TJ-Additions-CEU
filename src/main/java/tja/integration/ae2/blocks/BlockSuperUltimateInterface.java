...
import static gregtech.api.gui.widgets.tab.VerticalTabListRenderer.HorizontalLocation.RIGHT;
...
public class BlockSuperUltimateInterface extends BlockInterface {
    ...
    public boolean onActivated(final World world, final BlockPos pos, final EntityPlayer player, final EnumHand hand, final @Nullable ItemStack heldItem, final EnumFacing side, final float hitX, final float hitY, final float hitZ) {
        ...
        final TileSuperUltimateInterface superInterface = this.getTileEntity(world, pos);
    ...
    public static ModularUI createDualInterfaceGUI(TileEntityHolder holder, EntityPlayer player, ISuperDualInterface superDualInterface) {
        ...
        final WidgetTabBuilder tabBuilder = new WidgetTabBuilder()
                .setTabListRenderer(() -> new VerticalTabListRenderer(TOP, RIGHT))
                .addTab("tile.me.pattern_interface.name", TJBlocks.PATTERN_INTERFACE.maybeStack(1).orElse(ItemStack.EMPTY), widgets -> createPatternInterfaceTab(widgets, superDualInterface, buttonPopUpWidget, buttonPopUpTickWidget, multiPatternSlots))
    ...
    private static void createPatternInterfaceTab(List<Widget> tab, ISuperDualInterface superDualInterface, ButtonPopUpWidget<?> buttonPopUpWidget, ButtonPopUpWidget<?> buttonPopUpTickWidget, IItemHandler multiPatternSlots) {
        final DualitySuperInterface duality = (DualitySuperInterface) superDualInterface.getInterfaceDuality();
        ...
        final SlotScrollableWidgetGroup patternScrollableSlotGroup = new SlotScrollableWidgetGroup(7, 34, 166, 162, 9)
        ...
        for (int i = 0; i < duality.getPatterns().getSlots(); i++) {
            ...
            patternScrollableSlotGroup.addWidget(new AEPatternSlotWidget(duality.getPatterns(), i, 18 * (i % 9), 18 * (i / 9))
                    .setActiveSupplier(() -> index / 9 <= upgradeHandler.getInstalledUpgrades(Upgrades.PATTERN_EXPANSION) && buttonPopUpWidget.getIndex() == 0 && buttonPopUpTickWidget.getIndex() == 0)
        ...
        tab.add(new LabelWidget(7, 23, "gui.appliedenergistics2.Patterns"));
        tab.add(new TJToggleButtonWidget(-18, 8, 16, 16, () -> duality.getConfigManager().getSetting(Settings.BLOCK).ordinal() == 0, superDualInterface::setBlockingMode)
        ...
        tab.add(new TJCycleButtonWidget<>(-18, 26, 16, 16, Settings.UNLOCK.getPossibleValues(), () -> duality.getConfigManager().getSetting(Settings.UNLOCK), superDualInterface::setLockCrafting)
        ...
        tab.add(new TJToggleButtonWidget(-18, 44, 16, 16, () -> duality.getConfigManager().getSetting(Settings.INTERFACE_TERMINAL).ordinal() == 0, superDualInterface::setInterfaceTerminal)
                ...
                .useToggleTexture(true));
        tab.add(new TJToggleButtonWidget(-18, 62, 16, 16, () -> duality.getConfigManager().getSetting(Settings.OPERATION_MODE).ordinal() == 0, superDualInterface::setFluidPacket)
                ...
                .useToggleTexture(true));
        tab.add(new TJToggleButtonWidget(-18, 80, 16, 16, () -> duality.getConfigManager().getSetting(Settings.LEVEL_TYPE).ordinal() == 0, superDualInterface::setSplittingItemsFluids)
                ...
                .useToggleTexture(true));
        tab.add(new TJCycleButtonWidget<>(-18, 98, 16, 16,  Settings.CONDENSER_OUTPUT.getPossibleValues(), () ->  duality.getConfigManager().getSetting(Settings.CONDENSER_OUTPUT), superDualInterface::setBlockModeEx)
        ...
        tab.add(new TJToggleButtonWidget(-18, 116, 16, 16, () -> ((RCIConfigurableObject) duality).r$getConfigManager().getSetting(RCSettings.IntelligentBlocking).ordinal() == 0, superDualInterface::setIntelligentBlocking)
                ...
                .setInvertTexture(true)
                .useToggleTexture(true));
        tab.add(new TJToggleButtonWidget(-18, 134, 16, 16, () -> false, (BooleanConsumer) bool -> changePatternAmount(duality.getPatterns(), patternScrollableSlotGroup, m -> m * 2, () -> updatePatterns(duality.getPatterns())))
        ...
        tab.add(new TJToggleButtonWidget(-18, 152, 16, 16, () -> false, (BooleanConsumer) bool -> changePatternAmount(duality.getPatterns(), patternScrollableSlotGroup, m -> m / 2, () -> updatePatterns(duality.getPatterns())))
        ...
        tab.add(new TJToggleButtonWidget(-18, 170, 16, 16, () -> false, (BooleanConsumer) bool -> changePatternAmount(duality.getPatterns(), patternScrollableSlotGroup, m -> m * 3, () -> updatePatterns(duality.getPatterns())))
        ...
        tab.add(new TJToggleButtonWidget(-18, 188, 16, 16, () -> false, (BooleanConsumer) bool -> changePatternAmount(duality.getPatterns(), patternScrollableSlotGroup, m -> m / 3, () -> updatePatterns(duality.getPatterns())))
    ...
    private static void createInterfaceTab(List<Widget> tab, ISuperDualInterface superDualInterface, ButtonPopUpWidget<?> buttonPopUpWidget, ButtonPopUpWidget<?> buttonPopUpTickWidget) {
        ...
        final SlotScrollableWidgetGroup storageScrollableSlotGroup = new SlotScrollableWidgetGroup(7, 34, 166, 144, 9)
                .setScrollWidth(4);
        for (int i = 0; i < duality.getConfig().getSlots(); i++) {
            final TJPhantomItemSlotWidget phantomItemSlotWidget = new TJPhantomItemSlotWidget(18 * (i % 9), 36 * (i / 9), 18, 18, i, duality.getConfig())
                    .setBackgroundTextures(TJGuiTextures.SLOT_DOWN);
            storageScrollableSlotGroup.addWidget(phantomItemSlotWidget);
        ...
        tab.add(upgradeScrollableSlotGroup);
        tab.add(storageScrollableSlotGroup);
        tab.add(selectionWidgetGroup);
        tab.add(new TJToggleButtonWidget(-18, 8, 16, 16, () -> duality.getConfigManager().getSetting(Settings.BLOCK).ordinal() == 0, superDualInterface::setAutoPull)
    ...
    private static void createFluidInterfaceTab(List<Widget> tab, ISuperDualInterface superDualInterface, ButtonPopUpWidget<?> buttonPopUpWidget, ButtonPopUpWidget<?> buttonPopUpTickWidget) {
        ...
        for (int i = 0; i < duality.getConfig().getSlots(); i++) {
            final int index = i;
            storageScrollableSlotGroup.addWidget(new TJPhantomAEFluidSlotWidget(18 * (i % 9), 36 * (i / 9), 18, 18, i, duality.getConfig(), fluidStack -> ((IDualitySuperFluidInterface) duality).onFluidInventoryHasChanged(duality.getConfig(), index, null, null, null))
    ...
    private static ModularUI.Builder createPatternMultiToolGUI(ModularUI.Builder builder, ItemStack patternMultiTool, FilteredItemStackHandler multiUpgradeSlots, IItemHandler patternSlots, IItemHandler multiPatternSlots, NBTTagCompound invTag) {
        if (!patternMultiTool.isEmpty()) {
