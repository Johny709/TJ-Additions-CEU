...
public abstract class MixinDualityInterface implements IDualitySuperInterface {
    ...
    public IAEItemStack injectTheCraftedItems(ICraftingLink link, IAEItemStack acquired, Actionable mode) {
        final int slot = ((IMixinMultiCraftingTracker) this.craftingTracker).getTheSlot(link);
    ...
    public boolean useThePlan(int x, IAEItemStack itemStack) {
        ...
        try {
            ...
            if (((IMixinMultiCraftingTracker) this.craftingTracker).isItBusy(x) && itemStack.getStackSize() > 0 && ((iaeItemStack = this.gridProxy.getStorage().getInventory(AEApi.instance().storage().getStorageChannel(IItemStorageChannel.class)).getStorageList().findPrecise(itemStack)) == null || iaeItemStack.getStackSize() == 0)) {
